...
const Cart = () => {
  ...
  useEffect(() => {
    let price = cartItems?.reduce((total: number, item) => {
      return (total =
        total +
        item?.quantity *
  ...
  const handleCheckoutFunction = async () => {
    const structuredData = cartItems?.map((product) => ({
  ...
  return (
    <div>
      <div className="flex flex-row gap-8 w-8/12 m-auto">
        <div className="flex flex-col gap-5 mt-2 w-4/6">
          <h1 className="font-bold text-4xl text-start">CART</h1>
          <ul className="flex flex-col gap-2 items-center mb-10">
            {cartItems?.map((product) => (
              <li
                key={product?.id}
                className="flex flex-row gap-2 w-full border rounded-xl overflow-hidden p-2 h-36"
              >
                <img
                ...
                <div className="p-2 flex flex-col h-full w-full">
                  ...
                  <div className="flex flex-row items-center justify-between w-full">
                    ...
                    <div className="flex items-center text-sm">
                      <button
                        className="px-1 py-1 rounded-full bg-purple-500 text-white cursor-pointer hover:bg-purple-700 duration-300"
                        onClick={() =>
                          changeQuantity(
                            product?.product_id,
                            product?.quantity - 1
                      ...
                      <span className="text-black mx-2 text-sm font-medium">
                        {product?.quantity}
                      </span>
                      <button
                        className="px-1 py-1 rounded-full bg-purple-500 text-white cursor-pointer hover:bg-purple-700 duration-300"
        ...
        <div className="flex flex-col text-start mt-16 w-2/6">
          ...
          {isCheckoutLoading ? (
            ...
            <button
              className="w-full bg-black hover:bg-purple-600 duration-300 cursor-pointer p-2 text-white mt-5"
